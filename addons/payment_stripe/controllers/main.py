...
class StripeController(http.Controller):
    ...
    def _verify_signature(self, tx_sudo):
        ...
        :return: None
        :raise Forbidden: If the webhook secret is not configured, if the timestamp is too old, or
                          if the signatures don't match.
        """
        webhook_secret = stripe_utils.get_webhook_secret(tx_sudo.provider_id)
        if not webhook_secret:
            _logger.warning("Received webhook event with undefined webhook secret")
            raise Forbidden()
